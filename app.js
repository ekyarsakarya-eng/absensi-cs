...
// === TAMBAHAN LOAD LOKASI ===
async function loadLokasi(){
  try{
    const data = await api({action:'getLokasi'});
    LOCATIONS = {};
    data.forEach(l=> LOCATIONS[l.nama] = {lat:l.lat,lng:l.lng});
  }catch(e){ console.log('lokasi gagal',e); }
}
...
// === UBAH openC untuk tampilkan pilih lokasi ===
async function openC(t){
  cur=t;
  if(t==='in'){
    // tampilkan pilihan dari LOCATIONS
    const list = $("#lokasiList"); if(list){ list.innerHTML=''; Object.keys(LOCATIONS).forEach(n=>{ const b=document.createElement('button'); b.className='w-full py-3 mb-2 rounded-2xl bg-sky-600 text-white'; b.textContent=n; b.onclick=()=>{window.pilihLokasi=n;$("#lokasiModal").classList.replace('flex','hidden');lanjutKamera();}; list.appendChild(b); }); }
    $("#lokasiModal").classList.replace("hidden","flex");
    return;
  }
  lanjutKamera();
}
function lanjutKamera(){
  $("#camModal").classList.replace("hidden","flex");
  navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:1280}}).then(s=>{str=s;$("#video").srcObject=str}).catch(e=>{alert("Kamera gagal: "+e.message);closeC()});
  $("#camWatermark").classList.remove("hidden");$("#wmNama").textContent=user.nama;
  if(navigator.geolocation){navigator.geolocation.getCurrentPosition(p=>{userLoc=`${p.coords.latitude.toFixed(5)},${p.coords.longitude.toFixed(5)}`;$("#wmLokasi").textContent=(window.pilihLokasi||'')+' '+userLoc},()=>{userLoc='0,0';$("#wmLokasi").textContent="-"})}
  clearInterval(window.wmTimer);window.wmTimer=setInterval(()=>{$("#wmWaktu").textContent=fW(new Date).replace(/:/g,".")+" WIB"},500)
}
